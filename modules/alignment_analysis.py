import sys
from PyQt5.QtWidgets import (QApplication, QPushButton, QMainWindow,QComboBox, QLineEdit,QCheckBox,QTextBrowser,QMessageBox,QErrorMessage,QLabel)
...
class AlignmentAnalysis(QMainWindow):
    ...
    def input_buttons(self):
        alignment_file_pushbutton = QPushButton('Choose an alignment file',self)
        alignment_file_pushbutton.setGeometry(30,10,200,30)
        alignment_file_pushbutton.clicked.connect(self.get_path_alignment)
        information_file_pushbutton = QPushButton('Choose an information file',self)
    ...
    def ion_class(self):
        ...
        self.keys_menu_combobox.addItems(["No class filter",*self.ion_key_view])
        ...
        self.primary_class_menu_combobox.currentIndexChanged.connect(self.add_class_to_browser)
    ...
    def analysis_deepness(self):
        ...
        self.primary_class_menu_combobox.currentIndexChanged.connect(self.get_ion_class_deepness)
    ...
    def operation_type(self):
        ...
        self.deepness_combobox.currentIndexChanged.connect(self.class_inclusion)

    def data_previsualisation(self):
        data_previsualisation_pushbutton = QPushButton("Data previsualisation",self)
        data_previsualisation_pushbutton.setGeometry(310,260,240,30)
        data_previsualisation_pushbutton.clicked.connect(self.prepare_information)
        data_previsualisation_pushbutton.clicked.connect(self.prepare_df)
        data_previsualisation_pushbutton.clicked.connect(self.action_previsualisation)
    ...
    def get_ion_class(self):
        if self.keys_menu_combobox.currentText() != "No class filter":
            df = open_filter(self.keys_menu_combobox.currentText(),self.ion_key_names,"ion classes")
            self.primary_class_menu_combobox.clear()
            self.primary_class_menu_combobox.addItem('None')
            self.primary_class_menu_combobox.addItems([*df.iloc[:,0].unique()])
        else:
            self.primary_class_menu_combobox.clear()
            self.primary_class_menu_combobox.addItem("Select a class")
    ...
    def add_unitary_ion(self):
        if self.precise_ion_lineedit.text() not in self.all_ion:
            ...
            QMessageBox.about(self,"information","This mass is already in the list")
    ...
    def get_ion_class_deepness(self):
        if self.keys_menu_combobox.currentText() != "No class filter":
            df = open_filter(self.keys_menu_combobox.currentText(),self.ion_key_names,"ion classes")
            deepness = ["/".join(df.columns.to_list()[:i]) for i,j in enumerate(df.columns.to_list()) if i > 0]
    ...
    def class_inclusion(self):
        ...
        if self.deepness_combobox.currentText() != "None":
            ...
            for i in range(1,len(divider)+1):
                res = res + divider if i == 1 else res + list(combinations(divider,i))
            for j in res:
                res_2 = res_2 + [f'sum of {j}'] if type(j) is str else res_2 + [f'sum of {" ⊂ ".join(j[::-1])}']
    ...
    def prepare_df(self):
        try:
            ...
            if "alignment" not in buffer.sheet_names:
                ...
                self.output = header_normalizer(pd.read_excel(buffer,sheet_name=0,index_col=0))
                if self.all_data_box.isChecked() == True:
                    ...
                    df_unitary = unitary_ion_filter(self.output,self.all_ion,self.memory)
                    df_ion_from_class = ion_from_class_filter(self.output,open_filter(self.keys_menu_combobox.currentText(),self.ion_key_names,"ion classes"),self.all_ion,self.deepness_combobox.currentText())
                    df_ion_from_class = class_operation(df_ion_from_class,self.type_of_operation_combobox.currentText(),self.deepness_combobox.currentText())
                    self.output = pd.concat([df_unitary,df_ion_from_class], join = "outer",axis = 0)
                ...
                if self.information_df is not None and self.information_df.shape[1] >0:
                    self.output = pd.concat([self.output,self.information_df],join = "outer",axis = 0)
        except ValueError:
            err = QErrorMessage(self)
    ...
    def prepare_information(self):
        if self.information is None or self.information == "" or len(self.information) == 0:
            ...
            self.information_df = header_normalizer(pd.read_excel(self.information,index_col=0,sheet_name=0))
            return
    ...
    def action_launcher(self):
        if self.output.size > 0:
            self.plot_launcher = PlotLauncher(self.output,self.information_df,self.deepness_combobox.currentText(),self.type_of_operation_combobox.currentText(),self.all_data_box.isChecked())
