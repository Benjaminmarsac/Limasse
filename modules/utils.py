...
def open_filter(button_response : str, files_names: str, folder : str):
    if f"{button_response}.csv" in files_names:
        return pd.read_csv(f"{FULL_PATH}/{folder}/{button_response}.csv", delimiter=",")
    else:
...
def save_data(excel_output, df, sheet_name, header=None, mode="w"):
...
def ion_alignment(df_to_analyse : pd.DataFrame, matrix_for_alignment : pd.DataFrame):
    ...
    molecule = df_to_analyse.loc[:,~df_to_analyse.columns.isin(["mass","intensity","acceptation","percent"])]
    ion_percent = list()
    df_to_analyse.loc[:, 'code'] = molecule.apply(lambda x: "/".join([str(int(x[c])) for c in molecule]), axis=1) if molecule.shape[1] > 1 else molecule.iloc[:,0]
    for i in range(len(matrix_for_alignment.loc[:, 'code'])):
        transition = df_to_analyse[df_to_analyse.loc[:,'code'] == matrix_for_alignment.iloc[i, 0]]['percent']
...
def unitary_statistics(analyse : pd.DataFrame):
    ...
    analyse.loc[:, "intensity"] = analyse.loc[:,"intensity"] / analyse.loc[:, "acceptation"]
    intensity_sum = np.sum(analyse.loc[:, "intensity"])
    analyse.loc[:, "percent"] = (analyse.loc[:, "intensity"] / intensity_sum)*100
...
def alignment_matrix(key : pd.DataFrame):
    ...
    alignment = {}
    alignment["code"] = molecule.apply(lambda x: "/".join([str(int(x[c])) for c in molecule]), axis=1) if molecule.shape[1] > 1 else molecule.iloc[:,0]
    alignment["nomenclature"] = molecule.apply(lambda x: "".join([f"[{(x[c])}]{d}" for c,d in zip(molecule,molecule.columns.to_list())]), axis=1) if molecule.shape[1] > 1 else molecule.iloc[:,0]
...
def unitary_ion_filter(df: pd.DataFrame, ion_list : list,key : list):
    ...
    if buffer:
        ...
        for i in buffer:
            try:
                i = float(i)
                df_to_return.loc[:,'unitary ion'][(df_to_return.loc[:,"mass"].astype(float)*1.001 > i ) & (df_to_return.loc[:,"mass"].astype(float)*0.999 < i )] = "took"
            except ValueError:
                err = QErrorMessage()
    ...
    if ion_list and len(deepness) > 0:
        ...
        if ion_copy.shape[0] > 0:
            for i in range(ion_copy.shape[0]):
                inter = df_inter[df_inter.loc[:,"code"] == ion_copy.iloc[i,-1]]
                if inter.shape[0] > 1:
                    inter.reset_index(inplace=True)
                    inter = pd.concat([inter,pd.DataFrame({l:[ion_copy.iloc[i,k]]*inter.shape[0] for k,l in enumerate(new_columns)})],axis = 1).to_numpy()
                elif inter.shape[0] == 1:
                    inter = np.append(inter.to_numpy(),ion_copy.iloc[i,0:len(new_columns)].to_list())
...
def class_operation(df : pd.DataFrame,type_of_operation : str, deepness : str):
    ...
    if type_of_operation == "No operation":
        ...
        if len(type_of_operation) > 0:
            ...
            df_to_return.loc[:,~df_to_return.columns.isin(["mass","code","nomenclature",*deepness])].astype(float)
            df_to_return = check_for_ambiguity(df_to_return,deepness)
            df_to_return = df_to_return.groupby(by = [*type_of_operation]).sum()
            df_to_return.drop(columns=["mass","code","nomenclature"],inplace = True)
        return df_to_return
...
def prepare_df_to_plot(df : pd.DataFrame,type_of_operation : str, deepness :str, mode : str = "default"):
    ...
    if type_of_operation == "No operation" and mode == "default":
        df_to_return.drop(columns = ['mass','nomenclature','code',*deepness],inplace=True)
        df_to_return.dropna(axis = 1, inplace = True)
        return df_to_return
    elif type_of_operation != "No operation" or mode == "cluster":
        df_to_return.drop(columns = ['mass','nomenclature','code'],inplace=True)
        df_to_return.dropna(axis = 1, inplace = True)
        return df_to_return
...
def check_for_ambiguity(df : pd.DataFrame,deepness : list):
    ...
    is_ambiguous = list(set([i for i in ambiguous if sum([j == i for j in ambiguous]) > 1]))
    if is_ambiguous:
        for j in is_ambiguous:
            for i in deepness:
                if len(df.loc[:,i][df.loc[:,"code"] == j].unique()) > 1:
                    res = df.loc[:,i][df.loc[:,"code"] == j].unique()
                    values = df.loc[(df.loc[:,'code'] == j),~df.columns.isin(["code","mass","nomenclature",*deepness])].iloc[0,:]
                    lower = (df.loc[df.loc[:,i].isin(res),~df.columns.isin(["code","mass","nomenclature",*deepness])].astype(float).sum()+(1*10**-9)-values*(len(res)))
                    for k in res:
                        upper = (df.loc[(df.loc[:,i] == k)&(df.loc[:,'code'] != j) ,~df.columns.isin(["code","mass","nomenclature",*deepness])].astype(float).sum())
                        m = (upper/lower)*values
                        n = df.loc[(df.loc[:,i] == k)&(df.loc[:,'code'] == j) ,df.columns.isin(["code","mass","nomenclature",*deepness])]
                        m,n = m.to_dict(),n.to_dict()
                        m = pd.DataFrame(m | n)
                        df.drop(index = df.loc[(df.loc[:,i] == k)&(df.loc[:,'code'] == j) ].index,inplace=True)
