...
class Clustermap():
    ...
    def deepness_checker(self):
        if self.deepness is None:
            ...
            self.df['new_col'] = self.df[self.deepness].apply(
                "/".join, axis=1) if self.operation_type == "No operation" else self.df.index
            lut = dict(zip(self.df["new_col"].unique(), "bgrcmykw"))
            self.row_colors = self.df["new_col"].map(lut)
            self.df = self.df.drop(columns=[
                                   *self.deepness, 'new_col']) if self.operation_type == "No operation" else self.df.drop(columns=['new_col'])
