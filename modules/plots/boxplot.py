from PyQt5.QtWidgets import (QPushButton, QMainWindow,QComboBox, QLineEdit,QLabel)
...
class BoxPlot(QMainWindow):
    def __init__(self,df : pd.DataFrame,information : list):
        ...
        width, height = (self.button_width*number_of_window_width +60), int(((self.button_height+20)*number_of_window_height+20))
    ...
    def plot_parameters(self):
        ...
        self.type_parameter.addItems(["Box","Violin","Boxenplot"])
    ...
    def plot_button(self):
        color_parameter = self.color_parameter.currentText() if self.color_parameter.currentText() != "Empty" else None
        xlab = self.x_label.text() if len(self.x_label.text()) != 0 else self.x_parameter.currentText()
        ylab = self.y_label.text() if len(self.y_label.text()) != 0 else self.y_parameter.currentText()
        plot_title = self.plot_title.text() if len(self.plot_title.text()) != 0 else None
        fig, ax = plt.subplots(figsize=(6.5, 6.5))
        sns.despine(fig, left=True, bottom=True)
        if self.type_parameter.currentText() == "Box":
            sns.boxplot(x=self.x_parameter.currentText(),
                y=self.df.iloc[:,self.y_parameter.currentIndex()],
                data=self.df,
                ax=ax,
                hue = color_parameter)
        if self.type_parameter.currentText() == "Violin":
            sns.violinplot(x=self.x_parameter.currentText(),
                y=self.df.iloc[:,self.y_parameter.currentIndex()],
                data=self.df,
                ax=ax,
                hue = color_parameter)
        if self.type_parameter.currentText() == "Boxenplot":
            sns.boxenplot(x=self.x_parameter.currentText(),
                y=self.df.iloc[:,self.y_parameter.currentIndex()],
                data=self.df,
                ax=ax,
                hue = color_parameter)
